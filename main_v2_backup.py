...
Improvements over v1:
  1. Production-adjusted fleet sizing  — garrison grows during travel; account for it
  2. Global commitment tracking        — prevents over-committing multiple planets to same target
  3. Adaptive garrison                 — 20% base (down from 25%), scales up only when threatened
  4. Better target scoring             — value = production * remaining turns (discounts distant targets)
  5. Threat-aware defensive reinforce  — reinforces most at-risk planet, not just least-ships
"""
