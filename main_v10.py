"""
Orbit Wars — Agent v10
======================
Base: v2 (all logic identical)
Change: compute_send() — send more ships per fleet to match opponent fleet sizes.
  Early game (step < 120): max(needed, 50% available, 15)
  Late game  (step >= 120): max(needed, 60% available)
Motivated by replay analysis: losses happen at avg fleet 5.5 vs opponent 27+.
"""
...
BOARD_SIZE         = 100.0
MAX_SPEED          = 6.0
GARRISON_FRAC      = 0.20   # base: keep 20% of ships for defense
MIN_SHIPS_TO_SEND  = 3
GAME_LENGTH        = 500
...
def ships_to_capture(src: Planet, tgt: Planet, committed_already: int = 0) -> int:
    """
    Ships needed from src to flip tgt, accounting for garrison growth during transit.
    committed_already = friendly ships already en route to tgt.
    """
...
def score_target(src: Planet, tgt: Planet, turn: int, committed: int) -> float:
    ...
    # Production value weighted by turns remaining to benefit from it
    value = tgt.production * turns_left
    if tgt.owner == -1:
        value *= 1.4   # neutrals are cheaper — prioritize early expansion
...
# ---------------------------------------------------------------------------
# Fleet size: send more ships per launch to match strong opponents
# ---------------------------------------------------------------------------

def compute_send(available: int, needed: int, step: int) -> int:
    if step < 120:
        # Early game: send max(needed, half available)
        # Never send less than 15 if we have it
        return min(available, max(needed, int(available * 0.5), 15))
    else:
        # Late game: send 60% of available, minimum needed
        return min(available, max(needed, int(available * 0.6)))
...
def agent(obs):
    ...
    # Ships already en route to each target (friendly fleets launched in prior turns)
    committed = {}
    for f in fleets:
        if f.owner != player:
            continue
    ...
    # Updated this turn — prevents two planets from double-committing to the same target
    ...
    for mine in my_planets:
        ...
        if not scored:
            # Reinforce the most at-risk friendly planet
